...
{
    // Do the opposite of convertScreenToWorld, backwards and negated.
    // Un-translate with camera position
    worldCoords -= _position;
    // Un-scale the coordinates
    worldCoords *= _scale;
    // Make it so 0 is the bottom left.
    worldCoords += glm::vec2(_screenWidth / 2, _screenHeight / 2);
    // Invert Y direction (Same forwards and backwards)
    // worldCoords.y = _screenHeight - worldCoords.y;
    return worldCoords;
}

glm::vec2 Camera2D::convertWorldSizeToRelativeScreenSize(glm::vec2 worldSize)
{
	// Change to pixel scale
    worldSize *= _scale;
    // Get the *relative* size
	worldSize /= glm::vec2(_screenWidth, _screenHeight);
	return worldSize;
}
