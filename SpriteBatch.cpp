...
Glyph::Glyph(const glm::vec4& destRect,
    ...
    const ColourRGBA8& color,
    float angle,
    const glm::vec2& pointOfRotation)
    : texture(Texture)
    , depth(Depth)
{
...
void FrameBufferObject::begin()
{
    ...
    // glEnable(GL_ALPHA_TEST); // Make sure that we exclude completely transparent pixels (below). NO LONGER IN OPENGL
...
void FrameBufferObject::createVertexArray()
{
    ...
    Glyph g(m_destRect, glm::vec4(0.0f, 0.0f, 1.0f, 1.0f), 0, 0.8f,
        ColourRGBA8(255, 255, 255, 255)); // Just make a glyph to handle the vertex-placement
    ...
    glBufferData(GL_ARRAY_BUFFER, 6 * sizeof(Vertex), nullptr,
        GL_STATIC_DRAW); // GL_STATIC so that we can only read after this (for speed, baby!)
    // Upload the data
